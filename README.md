...
DALL-E 2 is a diffusion model developed by OpenAI. It works by learning to reverse a process of gradual noise. This means that during training, images are progressively corrupted until unrecognisable, and the model learns to reconstruct them step by step. This is combined with CLIP (Contrastive Language-Image Pre-training), which encodes text prompts into representations that guide what the diffusion model generates, allowing written descriptions to produce original images. DALL-E 2 was trained on a large dataset of image-text pairs drawn predominantly from digitised, Western-facing web content. In this project, the inputs were written text prompts alongside reference photographs of the tapa samples, and the outputs were newly synthesised 1024×1024 pixel images. In short, the model does not retrieve or modify existing images but instead, it generates entirely new ones.
...
Specifically, Microsoft Copilot's transformer architechture is extended with multimodal capability to process both image and text inputs, using a self-attention mechanism that weighs the relevance of every word or image patch in relation to every other simultaneously, enabling the model to understand context across complex inputs and generate coherent responses. Copilot was pre-trained on a large corpus of digitised text and further refined using reinforcement learning from human feedback (RLHF), where human evaluators rated response quality to train the model toward more helpful and accurate outputs. 
